...
public final class AndroidAppHelper {
    ...
    static {
        ...
        CLASS_RESOURCES_KEY = (Build.VERSION.SDK_INT < 19) ?
                Ref.findClass("android.app.ActivityThread$ResourcesKey", null)
                : Ref.findClass("android.content.res.ResourcesKey", null);

        HAS_IS_THEMEABLE = Ref.findFieldIfExists(CLASS_RESOURCES_KEY, "mIsThemeable") != null;
        HAS_THEME_CONFIG_PARAMETER = HAS_IS_THEMEABLE && Build.VERSION.SDK_INT >= 21
                && (Ref.findMethodExactIfExists("android.app.ResourcesManager", null, "getThemeConfig") != null);
    }
    ...
    private static Object createResourcesKey(String resDir, float scale) {
        try {
            if (HAS_IS_THEMEABLE)
                return Ref.newInstance(CLASS_RESOURCES_KEY, resDir, scale, false);
            else
                return Ref.newInstance(CLASS_RESOURCES_KEY, resDir, scale);
        } catch (Throwable t) {
            XL.e(t);
            return null;
        }
    ...
    private static Object createResourcesKey(String resDir, int displayId, Configuration overrideConfiguration, float scale) {
        try {
            if (HAS_THEME_CONFIG_PARAMETER)
                return Ref.newInstance(CLASS_RESOURCES_KEY, resDir, displayId, overrideConfiguration, scale, false, null);
            else if (HAS_IS_THEMEABLE)
                return Ref.newInstance(CLASS_RESOURCES_KEY, resDir, displayId, overrideConfiguration, scale, false);
            else
                return Ref.newInstance(CLASS_RESOURCES_KEY, resDir, displayId, overrideConfiguration, scale);
        } catch (Throwable t) {
            XL.e(t);
            return null;
        }
    ...
    private static Object createResourcesKey(String resDir, int displayId, Configuration overrideConfiguration, float scale, IBinder token) {
        try {
            if (HAS_THEME_CONFIG_PARAMETER)
                return Ref.newInstance(CLASS_RESOURCES_KEY, resDir, displayId, overrideConfiguration, scale, false, null, token);
            else if (HAS_IS_THEMEABLE)
                return Ref.newInstance(CLASS_RESOURCES_KEY, resDir, displayId, overrideConfiguration, scale, false, token);
            else
                return Ref.newInstance(CLASS_RESOURCES_KEY, resDir, displayId, overrideConfiguration, scale, token);
        } catch (Throwable t) {
            XL.e(t);
            return null;
        }
    ...
    private static Object createResourcesKey(String resDir, String[] splitResDirs, String[] overlayDirs, String[] libDirs, int displayId, Configuration overrideConfiguration, CompatibilityInfo compatInfo) {
        try {
            return Ref.newInstance(CLASS_RESOURCES_KEY, resDir, splitResDirs, overlayDirs, libDirs, displayId, overrideConfiguration, compatInfo);
        } catch (Throwable t) {
            XL.e(t);
            return null;
        }
    ...
    public static void addActiveResource(String resDir, Resources resources) {
        ...
        Object resourcesKey;
        if (Build.VERSION.SDK_INT >= 24) {
            CompatibilityInfo compatInfo = (CompatibilityInfo) Ref.newInstance(CompatibilityInfo.class);
            Ref.setFloatField(compatInfo, "applicationScale", resources.hashCode());
    ...
    /** @deprecated Use {XSharedPreferences} instead. */
    @SuppressWarnings("UnusedParameters")
    @Deprecated
    public static SharedPreferences getSharedPreferencesForPackage(String packageName, String prefFileName, int mode) {
        return (SharedPreferences) Ref.newInstance("de.robv.android.xposed.XSharedPreferences", packageName, prefFileName);
//         new XSharedPreferences(packageName, prefFileName);
    }
...
//        return new XSharedPreferences(packageName);
        return (SharedPreferences) Ref.newInstance("de.robv.android.xposed.XSharedPreferences", packageName);
    }

    /** @deprecated Use {XSharedPreferences#reload} instead. */
    @Deprecated
...
//        }
        ...
        if (Subclass.isSubClass(pref.getClass(), "de.robv.android.xposed.XSharedPreferences")) {
            Ref.callMethod(pref, "reload");
        }
